...
//----------------------------------------------------------------------------
  ...
  static inline unsigned long millis(void) {
    long current_time = clint_getTime(BSP_CLINT);
    return (long)(current_time / (SYSTEM_CLINT_HZ / 1000));
  }

  static inline unsigned long micros(void) {
    long current_time = clint_getTime(BSP_CLINT);
    return (long)(current_time / (SYSTEM_CLINT_HZ / 1000000));
  }

  static inline void delay(unsigned long milliseconds) {
    bsp_uDelay(milliseconds * 1000);
  }

  static inline void delayMicroseconds(unsigned int us) { bsp_uDelay(us); }
